(function () {
  ...
  document.addEventListener("click", function (event) {
    ...
    if (/^https:\/\/(wa\.me|api\.whatsapp\.com)\//i.test(href)) {
      track("click_whatsapp", { contact_method: "whatsapp", link_url: href });
      track("contact_click", { contact_method: "whatsapp" });
    } else if (/^tel:/i.test(href)) {
      track("click_phone", { contact_method: "phone", link_url: href });
  ...
  document.addEventListener("submit", function (event) {
    ...
    if (["quoteForm", "partnerForm"].includes(form.id)) {
      const formName = form.id === "quoteForm" ? "preventivo" : "collaborazione";
      track("generate_lead", { form_name: formName, currency: "EUR", value: 1 });
      track("whatsapp_form_submit", { form_name: formName });
    }
  });
